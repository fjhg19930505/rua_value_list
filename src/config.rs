...
impl Config {
    ...
    pub fn new_by_full_str(config: &str) -> Option<Config> {
        ...
        let field = json::decode(&field.unwrap().to_string());
        let proto = json::decode(&proto.unwrap().to_string());
    ...
    pub fn new(field: &str, proto: &str) -> Option<Self> {
        let field = json::decode(field);
        let proto = json::decode(proto);
